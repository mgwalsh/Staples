...
<br>
```{r, echo=FALSE, fig.align="center", fig.cap="**Main differences between binary, multiclass and multilabel data types.**", out.width = '75%'}
...
```{r}
# Package names
packages <- c("osfr", "tidyverse", "leaflet", "arules", "arulesViz", "rgdal", "sp", "raster",
              "doParallel", "caret", "randomForest")
...
The survey data and figures that are used in this notebook can be downloaded with the next chunk ... in theory. However, at the moment and for reasons beyond our control, you need to download the data manually into your working directory from [here](https://osf.io/2unc3/).
...
```{r, echo = FALSE, fig.align = "center", out.width = "80%", fig.cap = "**Figure 6:** Association rule graph of the main staple crop combinations found in close proximity to rural buildings in Rwanda and Tanzania."}
...
<br>
```{r crop_distributions, echo = FALSE, fig.align = "center", fig.cap="**Figure 8:** Stacked probability predictions (in green or red) of the main staple food crop distributions in Rwanda and Tanzania. Note that the areas in white fall outside of our cropland region of interest.", out.width = '95%'}
...
[ROC](https://people.inf.elte.hu/kiss/11dwhdm/roc.pdf) graphs are used to provide information about classifier test performance on unseen data. The closer the apex of the curve toward the upper left corner, the greater the discriminatory  ability of the test (i.e., the  true-positive rate is high and the false-positive rate is low. We use it here to assess the classification performance of the individual stacked models of the **calibration set**. The next chunks do the calculations and plot the ROC curves using the [`dismo`](https://cran.r-project.org/web/packages/dismo/dismo.pdf) package. Areas under the ROC curve ([AUC](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)) of 1 discriminate perfectly. AUC values of 0.5 are no better than a random guess. The next chunk sets this up.

```{r, results = 'hide'}
# Load stacked learner rasters
file.remove("./Results/maiz_st.tif.aux.xml")
...
* We initially used an association rule (AR) mining workflow to explore staple food crop combinations that are both prevalent and potentially informative; that is, ARs that exhibit adequate support, confidence and lift to consider them to be of generalizable interest for identifying potential staple crop diversification pathways in both countries.

* Rwanda's staple food production systems appear to be more diverse than those typically found in Tanzania. However, this observation appears to be largely related to differences in rural population densities in the two countries, as indicated by differences in the proximity to buildings. Staple food cropping systems in proximity to buildings will generally be more complex in both Rwanda and Tanzania.

* Notably, there are 2^20^-1 potential staple crop combinations that might occur at random with 20 indicator species. One could spend a lot of time generating all those *potential* combinations with SQL queries. We have identified ..., which are both prevalent and potentially interesting for agricultural and human dietary diversity management. That part of the code in the notebook runs in < 1 minute on a normal laptop.
...
* Section 4 (Figure 8) shows spatial predictions of where potentially interesting, mixed cropping systems occur in Rwanda and Tanzania.
